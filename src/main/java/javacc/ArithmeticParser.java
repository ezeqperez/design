...
public class ArithmeticParser implements ArithmeticParserConstants {
  ...
  final public void INDICADOR(Indicador indicador) throws ParseException {
    POPERANDO(indicador);
    OPERACION(indicador);
    SOPERANDO(indicador);
  }

  final public void POPERANDO(Indicador indicador) throws ParseException {
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case NUMBER:{
      jj_consume_token(NUMBER);
indicador.setPrimerOperando(new Valor(token.image));
      break;
      }
    case CUENTA:{
      jj_consume_token(CUENTA);
indicador.setPrimerOperando(new CuentaOperando(token.image));
      break;
      }
    default:
      jj_la1[0] = jj_gen;
  ...
  final public void OPERACION(Indicador indicador) throws ParseException {
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case MULTIPLY:{
      jj_consume_token(MULTIPLY);
indicador.setOperacion(new Multiplicacion());
      break;
      }
    case DIVIDE:{
      jj_consume_token(DIVIDE);
indicador.setOperacion(new Division());
      break;
      }
    case ADD:{
      jj_consume_token(ADD);
indicador.setOperacion(new Suma());
      break;
      }
    case SUBTRACT:{
      jj_consume_token(SUBTRACT);
indicador.setOperacion(new Resta());
      break;
      }
    default:
      jj_la1[1] = jj_gen;
  ...
  final public void SOPERANDO(Indicador indicador) throws ParseException {
    if (jj_2_1(2)) {
      SUBINDICADOR(indicador);
    } else if (jj_2_2(2)) {
      jj_consume_token(NUMBER);
indicador.setSegundoOperando(new Valor(token.image));
    } else {
      switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
      case CUENTA:{
        jj_consume_token(CUENTA);
indicador.setSegundoOperando(new CuentaOperando(token.image));
        break;
        }
      default:
        jj_la1[2] = jj_gen;
  ...
  final public void SUBINDICADOR(Indicador indicador) throws ParseException {Indicador subIndicador = new Indicador();
    indicador.setSegundoOperando(subIndicador);
    POPERANDO(subIndicador);
    OPERACION(subIndicador);
    SOPERANDO(subIndicador);
  }
